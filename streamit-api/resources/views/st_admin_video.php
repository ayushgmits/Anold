<?php
// $videoData = get_option('streamit_app_video');
// $videoList = getSTVideoList()->toArray();
// $getSTTagList = getSTVideoTagList()->toArray();
// $getSTCategoryList = getSTVideoCategoryList()->toArray();
// $getSTFilterList = getSTFilterList()->toArray();


$videoData = get_option('streamit_app_video');
// $videoList = getSTTVShowList()->toArray();
$dynamicShowType = get_option('default_show_type', 'video_audio_series'); // Fetch from options or another source
$videoList = getSTTVShowList(false, $dynamicShowType)->toArray();
$getSTCategoryList = getSTTVShowGenreList()->toArray();
$getSTTagList = getSTTVShowTagList()->toArray();
$getSTFilterList = getSTFilterList()->toArray();
?>
<div class="card p-0">
    <div class="card-body">
        <form name="st-admin-option-video" id="st-admin-option-video" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-12">
                    <h5>
                        <?php echo esc_html__('Video Banner', 'streamit-plugin-lang') ?>
                    </h5>
                    <hr>
                </div>
                <div class="col-lg-2">
                ...
                <div class="col-lg-10">
                    <div id="movie-banner-slide" class="st-clone-master" data-accordion="true">
                        <?php
                        if (isset($videoData['banner']) && count($videoData['banner']) > 0) {
                            foreach ($videoData['banner'] as $i1 => $d1) {
                                ?>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <select class="form-control st-multiple-checkboxes"
                                                    ...
                                                    foreach ($videoList as $video) {
                                                        echo '<option value="' . $video['value'] . '" >' . $video['text'] . '</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                ...
                <div class="col-lg-10">
                    <div id="movie-sub-slider" class="st-clone-master" data-accordion="true">
                        <?php
                        if (isset($videoData['sliders']) && count($videoData['sliders']) > 0) {
                            foreach ($videoData['sliders'] as $i2 => $d2) { ?>
                                <div class="card st-clone-item">
                                    <div class="card-header st-accordion-header collapsed"
                                        ...
                                        <div class="card-body">
                                            <div class="row">
                                                ...
                                                <div class="col-lg-6 mt-3">
                                                    ...
                                                    <select class="form-control st-multiple-checkboxes"
                                                            data-actions-box="true"
                                                            data-live-search="true" data-size="10" name="sub_cat[]" multiple
                                                            x-placement="Select Category">
                                                        <?php
                                                        foreach ($getSTCategoryList as $cat) {
                                                            echo '<option value="' . $cat['value'] . '" ' . (in_array($cat['value'], $d2['cat']) ? 'selected' : '') . '>' . $cat['text'] . '</option>';
                                                        }
                                                        ?>
                                                    </select>
                                                </div>
                                                ...
                                                <div class="col-lg-6 mt-3">
                                                    ...
                                                    <select class="form-control st-multiple-checkboxes" multiple
                                                        ...
                                                        <?php
                                                            foreach ($videoList as $video) {
                                                                echo '<option value="' . $video['value'] . '" ' . (in_array($video['value'], $d2['select_movie_show']) ? 'selected' : '') . ' >' . $video['text'] . '</option>';
                                                            }
                                                        ?>
                                                    </select>
                                                </div>
                ...
                <div class="col-md-12">
                    <hr class="mb-3">
                    <button type="button" class="btn btn-info mt-2" id="st-video-admin-setting">
